          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-blue-50 p-4 rounded-lg text-center">
              <div className="text-2xl font-bold text-blue-600 mb-2">
                {recordingTime > 0 ? `${Math.round((classAnalysis.voiceAnalysis.studentSpeechTime / recordingTime) * 100)}%` : '--'}
              </div>
              <p className="text-blue-700 font-medium text-sm">Tiempo de Participación Estudiantil</p>
            </div>
            <div className="bg-green-50 p-4 rounded-lg text-center">
              <div className="text-2xl font-bold text-green-600 mb-2">
                {recordingTime > 0 ? `${Math.round((classAnalysis.voiceAnalysis.professorSpeechTime / recordingTime) * 100)}%` : '--'}
              </div>
              <p className="text-green-700 font-medium text-sm">Tiempo de Instrucción Docente</p>
            </div>
            <div className="bg-purple-50 p-4 rounded-lg text-center">
              <div className="text-2xl font-bold text-purple-600 mb-2">
                {recordingTime > 0 ? Math.round(classAnalysis.voiceAnalysis.questionCount / (recordingTime / 60)) : '--'}
              </div>
